<template>
  ...
  >
    <div class="rounded-t bg-white mb-0 px-6 py-6" style="background-color: #303030;">
      <div class="text-center flex justify-between">
        <h6 class="text-blueGray-700 text-xl font-bold " style="color:white;">Get Free Vehicle Valuation</h6>

      </div>
    </div>
    <div class="flex-auto px-4 lg:px-10 py-10 pt-0" style="background-color: #202020;">
      <form method="post" action="/details" @submit.prevent="firebaseDetailsSubmit">
        <h6 class="text-blueGray-400 text-sm mt-3 mb-6 font-bold uppercase form-91" style="color:white;">
          Your Details
        </h6>
        ...
        <div class="form-91" style="display: grid; place-items: center;">  
          <button
            type="submit"
        ...
        <div class="container-89">
          <div class="form-sent-89"><br/>
            <p style="color:#bb86fc;">{{ formSent }}<a class="form-92" style="display:none;color:#bb86fc;font-weight:bold;" href="/">Home Page</a></p>
            <!-- {{ api1 }} -->
          </div>
        </div>
...
export default { 
  ...
  methods:{
    async firebaseDetailsSubmit () {
      ...
      var client = "{"+
        "\"Car Registration Number\": \""+this.regNo+"\", "+
        // "\"Car Make\": \""+this.make+"\", "+
        // "\"Car Model\": \""+this.model+"\", "+
        // "\"Car Color\": \""+this.color+"\", "+
        "\"Client Name\": \""+this.$refs.fullName.value+"\", "+
        "\"Client Email\": \""+this.$refs.email.value+"\", "+
        "\"Client Mobile\": \""+this.$refs.mobile1.value+"\", "+
        "\"Client Address\": \""+this.$refs.address.value+"\", "+
        "\"Client City\": \""+this.$refs.city.value+"\", "+
        "\"Client Country\": \""+this.$refs.country.value+"\", "+
        "\"Client Post Code\": \""+this.$refs.post.value+"\", "+
        "\"Car ID\": \""+(new Date()).getTime()+"\", "+
        "\"Car Timestamp\": \""+(new Date())+"\", "+
        "\"Client Details\": \""+this.$refs.details.value+"\""+
        "}";
      // console.log(vehicle);
      ...
      await db
        ...
        .add(client)
        .then(()=>{
          console.log("Client Details Data entered into Google Cloud DB");
          ...
          this.formSent = "Your message was sent successfully. We will get back at you asap. Meanwhile you may visit the ";
        }
        )
        .catch(e => console.log(e));
    },
    ...
    async API1(){
      var api1 = this.$cookies.get('api1');
      const db = firebase.firestore();
      await db
        .collection('API1').add(api1).then(()=>{
